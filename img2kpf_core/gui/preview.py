...
from ..kpf_generator import (
    ...
    horizontal_anchor_for_outer_edge,
    is_spread_crop_mode,
    load_pillow,
    load_source_image,
...
def _reference_blank_size(
    image_paths: list[Path],
    processed_cache: dict[int, tuple[object, object, object, tuple[int, int, int, int] | None]],
    fallback_index: int,
    image_processing: ImageProcessingOptions,
) -> tuple[int, int]:
...
def _build_preview_page(
    ...
    if image_index is None:
        ...
        return PreviewPage(
            ...
            crop_overlay_image=source_image.copy(),
    ...
    processed, crop_overlay, source, crop_box = processed_cache[image_index]
    return PreviewPage(
        page_number=image_index + 1,
        label=_tr("ui.page", language, page=image_index + 1),
        source_image=source,
        crop_overlay_image=crop_overlay,
...
def _build_processed_cache(
    ...
    image_processing: ImageProcessingOptions,
    layout_options: LayoutOptions,
) -> dict[int, tuple[object, object, object, tuple[int, int, int, int] | None]]:
    cache: dict[int, tuple[object, object, object, tuple[int, int, int, int] | None]] = {}
    ...
    if layout_options.page_layout != "facing" or not is_spread_crop_mode(image_processing.crop_mode):
        for slot in source_slots:
            ...
            processed, crop_overlay, crop_box = _process_basic_page(source, image_processing)
            cache[index] = (processed, crop_overlay, source, crop_box)
        return cache

    if len(source_slots) == 1:
        slot = source_slots[0]
        source = load_source_image(image_paths[slot.source_index])
        processed, crop_overlay, crop_box = _process_facing_single_page(
            source,
            image_processing,
            template_direction=layout_options.reading_direction,
            page_position=slot.page_position,
        )
        cache[slot.source_index] = (processed, crop_overlay, source, crop_box)
        return cache
    ...
    for slot, source, (processed, crop_overlay, crop_box) in zip(source_slots, sources, processed_entries):
        cache[slot.source_index] = (processed, crop_overlay, source, crop_box)
    return cache


def _process_basic_page(
    source_image,
    image_processing: ImageProcessingOptions,
) -> tuple[object, object, tuple[int, int, int, int] | None]:
    ...
    return processed, crop_overlay, crop_box
...
def _process_spread_group(
    ...
    image_processing: ImageProcessingOptions,
    template_direction: str,
) -> list[tuple[object, object, tuple[int, int, int, int] | None]]:
    ...
    results: list[tuple[object, object, tuple[int, int, int, int] | None]] = []
    for source, crop_box, page_position in zip(sources, synchronized_boxes, ("first", "second")):
        results.append(
            (
                _finalize_processed_image(
                    apply_crop_box(source.copy(), crop_box),
                    image_processing,
                    horizontal_anchor=horizontal_anchor_for_outer_edge(page_position, template_direction),
                ),
                _build_crop_overlay_image(source, image_processing),
                crop_box,
            )
        )
    return results
...
def _finalize_processed_image(image, image_processing: ImageProcessingOptions, horizontal_anchor: str = "center"):
    processed = apply_luminance_operations(image, image_processing)
    if image_processing.target_size is not None:
        processed = fit_image_to_canvas(
            processed,
            image_processing.target_size,
            image_processing.preserve_color,
...
def _build_crop_overlay_image(source_image, image_processing: ImageProcessingOptions):
    overlay = apply_luminance_operations(source_image.copy(), image_processing)
    if not image_processing.preserve_color:
...
def _render_row(
    ...
    if show_crop_boxes:
        ...
        for crop_rect in crop_rects:
            ...
            line_width = max(2, round(min(row_width, row_height) / 520))
            draw.rectangle(scaled_crop_rect, outline=PREVIEW_CROP, width=line_width)
    return row
...
def _spread_image_size(
    ...
    if image_kind == "processed":
        images = [page.processed_image for page in pages]
    elif image_kind == "crop-overlay":
        images = [page.crop_overlay_image for page in pages]
    else:
        images = [page.source_image for page in pages]
    if not images:
        return (1, 1)
...
def _compose_spread_image(
    ...
    for page in pages:
        if image_kind == "processed":
            image = page.processed_image
        elif image_kind == "crop-overlay":
            image = page.crop_overlay_image
        else:
