...
def _ensure_plugin_importable(plugin_zip_path: Path) -> None:
    if not plugin_zip_path.is_file():
        raise FileNotFoundError(
            ...
            "`img2kpf_core/plugins/kfx_output/`, or pass an explicit `--plugin` / `--kfx-plugin` path."
        )
...
def convert_kpf_to_kfx(
    ...
    logger = _build_logger("img2kpf_core.kfx_direct")
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help="Plugin ID, plugin directory, or plugin zip path; default expects `img2kpf_core/plugins/kfx_output/KFX Output.zip`.",
    )
